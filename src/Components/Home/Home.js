...
function Home({ cart }) {
    ...
    const filteredItem = data.shirts.filter(product => {
        if (query === "") {
            if (selected !== null) {
                if (product.gender === selected || product.color === selected || product.size === selected || product.type === selected || (product.price <= selected && product.price >= selected - 250)) return product;
            } else {
                return product;
            }
        ...
        <div>
            <div>
                ...
                <div className='flex gap-28'>
                    <div className='ml-12 flex flex-col border-2 pr-20 pl-6 pt-6 bg-slate-300 rounded-2xl min-h-screen pb-3'>
